...
// // import { getExpiryReport } from "@/app/services/ExpiredReportService";
...
//       try {
//         const response = await getExpiryReport();
//         if (!response || !Array.isArray(response)) {
//           throw new Error("Failed to fetch expiry report data");
//         }
...
//         const reportsWithStatus = response.map(item => {
...
//           } else {
//             expiryStatus = `Expiring in ${daysLeft} day${daysLeft > 1 ? "s" : ""}`;
//           }
...
//         setExpiryReport(reportsWithStatus);
//       } catch (error) {
...
//         // Special sorting for expiry status
//         if (sortConfig.key === "expiryStatus") {
//           const getStatusValue = (status: string) => {
//             if (status === "Expired") return 0;
//             if (status === "Expiring Today") return 1;
//             const daysMatch = status.match(/(\d+)/);
//             return daysMatch ? parseInt(daysMatch[0]) + 2 : Infinity;
//           };

//           const aVal = getStatusValue(a.expiryStatus);
//           const bVal = getStatusValue(b.expiryStatus);
...
//           isSameDay(new Date(item.expiryDate), today)
//         );
//       case "yesterday":
//         return data.filter((item) =>
//           isSameDay(new Date(item.expiryDate), subDays(today, 1))
//         );
//       case "thisWeek":
//         return data.filter((item) =>
...
//         <div
//           className="flex items-center gap-2 cursor-pointer"
//           onClick={() => handleSort("expiryStatus")}
//         >
//           <span>Expiring Stock</span>
//           {sortConfig.key === "expiryStatus" ? (
//             sortConfig.direction === "asc" ? (
//               <FaArrowUp />
//             ) : (
//               <FaArrowDown />
...
//         const status = row.expiryStatus.toLowerCase();
//         const textClass =
//           status.includes("expiring today")
//             ? "text-warning"
...
//         item.expiryStatus?.toLowerCase().includes(search)
//       );
//     })
//   );
...
//           { value: "expiredStock", label: "Expired Stock" },
//           { value: "custom", label: "Expiring Date Range" },
//         ].map((filter) => (
...
//         data={getSortedData()}
//         columns={columns}
//         noDataMessage="No expiry records found"
//       />
//     </main>
//   );
// };
